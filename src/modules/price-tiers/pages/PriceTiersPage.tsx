...
export function PriceTiersPage() {
  ...
  const columns = [
    ...
    { key: 'isActive', header: 'Estado', render: (item: PriceTier) => <span className={`px-2 py-1 rounded-full text-xs ${item.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{item.isActive ? 'Activo' : 'Inactivo'}</span> },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700"><Plus size={18} /> Nuevo Rango</button>
      </div>
