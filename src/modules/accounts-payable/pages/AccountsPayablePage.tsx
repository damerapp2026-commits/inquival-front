...
export function AccountsPayablePage() {
  ...
  const openPayment = (ap: AccountPayable) => {
    ...
    setPayForm({ amount: nextInst ? nextInst.amount : 0, codigoTransferencia: '', notes: '' });
    ...
    <div>
      ...
      <Modal isOpen={showPayModal} onClose={() => setShowPayModal(false)} title="Registrar Pago a Proveedor">
        <form onSubmit={handlePayment} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Código de transferencia (opcional)</label>
            <div className="relative">
              <Hash size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input
                type="text"
                value={payForm.codigoTransferencia}
                onChange={(e) => setPayForm({ ...payForm, codigoTransferencia: e.target.value })}
                placeholder="Ej: 00123456789"
                className="w-full pl-9 pr-3 py-2 border rounded-lg font-mono text-sm"
              />
            </div>
          </div>
          <div>
