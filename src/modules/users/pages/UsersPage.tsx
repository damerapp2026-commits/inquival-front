    ...
    },
    {
      key: 'actions', header: 'Acciones', render: (item: User) => (
        <div className="flex gap-2">
          ...
          <button onClick={() => toggleStatus.mutate(item.id)} className={`${item.isActive ? 'text-red-600 hover:text-red-800' : 'text-green-600 hover:text-green-800'}`} title={item.isActive ? 'Desactivar' : 'Activar'}><Power size={16} /></button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex gap-3 flex-wrap">
        ...
        <select value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value); setPage(1); }} className="px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
          <option value="">Todos los roles</option>
          <option value="ADMIN">Admin</option>
          <option value="VENDEDOR">Vendedor</option>
        </select>
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="Nuevo Usuario">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <button type="submit" disabled={createUser.isPending} className="w-full py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
      ...
      <Modal isOpen={!!editingUser} onClose={() => setEditingUser(null)} title="Editar Usuario">
        <form onSubmit={handleEdit} className="space-y-4">
          ...
          <button type="submit" disabled={updateUser.isPending} className="w-full py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
      ...
      <Modal isOpen={!!passwordUser} onClose={() => setPasswordUser(null)} title={`Cambiar contrasena - ${passwordUser?.fullName}`}>
        <form onSubmit={handlePasswordChange} className="space-y-4">
          ...
          <button type="submit" disabled={changePassword.isPending} className="w-full py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
