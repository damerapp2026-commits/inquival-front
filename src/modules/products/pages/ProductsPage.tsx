...
export function ProductsPage() {
  ...
  const handleShareCatalog = async () => {
    const url = `${window.location.origin}/lista-productos`;
    const text = `Mira nuestro catálogo de productos: ${url}`;
    const waUrl = `https://wa.me/?text=${encodeURIComponent(text)}`;
    const opened = window.open(waUrl, '_blank', 'noopener,noreferrer');

    if (!opened) {
      await navigator.clipboard.writeText(url).catch(() => undefined);
      toast('El navegador bloqueó WhatsApp. Link del catálogo copiado.', { icon: '📋', duration: 4000 });
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        {view === 'list' && (
          <div className="flex flex-wrap gap-2">
            <button onClick={handleShareCatalog} className="flex items-center gap-2 px-3 py-2 border border-green-300 text-green-700 bg-green-50 rounded-lg hover:bg-green-100 text-sm">
